...
async function openFeatureTourFromMenu(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ BrowserWindow, Menu }) => {
    const featureTourItem = Menu.getApplicationMenu()
      ...
      ?.submenu?.items.find((item) => item.label === 'Explore Tobasco')

    if (!featureTourItem) {
      throw new Error('Explore Tobasco menu item was not registered')
    }
...
test.describe('Feature tour modal', () => {
  ...
  test('opens from the Help menu and renders the workflow rail', async ({
    ...
    await expect(orcaPage.getByRole('dialog', { name: 'Get to know Tobasco' })).toBeVisible({
      timeout: 10_000
    })
    await expect(orcaPage.getByText('Reopen any time from Help > Explore Tobasco.')).toBeVisible()
    ...
    await expect(
      orcaPage.getByText(
        "Run your app in Tobasco's browser, send selected UI elements to agents, and let your agents interact with your webpage."
      )
    ).toBeVisible()
    await expect(orcaPage.getByRole('heading', { name: 'Browser Use skill' })).toBeVisible()
    await expect(
      orcaPage.getByText("Enables agents to navigate and verify pages in Tobasco's browser.")
    ).toBeVisible()
    await expect(orcaPage.getByRole('heading', { name: 'CLI skill' })).toHaveCount(0)
    await expect(orcaPage.getByText('With the Tobasco CLI skill', { exact: false })).toHaveCount(0)
  })
